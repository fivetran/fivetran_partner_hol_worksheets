...
Thank you for registering for our hands-on lab. This guide has everything you need to
prepare and follow along. Please read through the requirements first. If you can't meet
them, let us know and we'll get you rebooked on another workshop.

Current to **July 2026** documentation. Reference:
[fivetran.com/docs/connector-sdk](https://fivetran.com/docs/connector-sdk).
...
- Confirm you received the invite. It's sent to the email you used to register and
  comes from `notifcations@fivetran.com`.
    - If you did not recieve one, you likely already have a Fivetran account. So you can proceed with logging in with your email and password.
...
1. Log in to Fivetran at [https://fivetran.com/login](https://fivetran.com/login).
   - If you just signed up, use the username and password you created.
...
10. Create a `test.env` file in the root of your project:
    ...
    **Windows (PowerShell)**
    ...
    New-Item test.env
    ```
11. Open the `test.env` file in your code editor

11. Add the following to `test.env`, pasting in the API key you copied:

    ```
    FIVETRAN_API_KEY=<paste-your-base64-api-key-here>
    ```

12. Load the env file into your terminal session:
...
2. Open the Fivetran **Connector SDK Quickstart** examples on GitHub:
   [github.com/fivetran/fivetran_connector_sdk](https://github.com/fivetran/connector_sdk/blob/main/examples/quickstart_examples/hello/connector.py)
...
1. In the command below, replace `firstname_lastname` with your actual first and last
   ...
   fivetran deploy \
     ...
     --connection {firstname_lastname}_weather \
...
3. Click your connection and start the initial sync. It should take about two minutes.
...
def update(configuration: dict, state: dict):
    ...
    try:
        if "to_ts" in state:
            ...
            from_ts = (datetime.datetime.now() - datetime.timedelta(days=7)).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )

        to_ts = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
