# Hands-on Lab: Programmatic Management (SQL Server)
...
- The lab uses a **Snowflake** data warehouse (destination) and a **SQL Server** database (source).
...
- Create a SQL Server connection and trigger an initial sync with the REST API.
...
3. Paste in the payload below. (Generated from the REST API reference for *Create a Connection* with
   `sql_server` as the type, minimized for this lab.)
   - Replace `<your_group_id>` with the `id` from Part 4, Step 6.
   - In `schema_prefix`, replace `firstname_lastname` with your actual first and last name.
   - Replace `SQL_SERVER_PASSWORD` with the password from the 1Password link.
   - Replace `SQL_SERVER_HOSTNAME` with the hostname from the 1Password link.
   - Replace `SQL_SERVER_DATABASE` with the database name from the 1Password link.
