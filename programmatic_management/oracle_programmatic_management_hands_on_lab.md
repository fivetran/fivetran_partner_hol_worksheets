# Hands-on Lab: Programmatic Management (Oracle)
...
- The lab uses a **Snowflake** data warehouse (destination) and a **Oracle** database (source).
...
- Create a Oracle connection and trigger an initial sync with the REST API.
...
3. Paste in the payload below. (This is generated from the REST API reference for *Create a
   ...
   - Replace `SNOWFLAKE_PRIVATE_KEY` with the private key from the 1Password link. JSON does
     not allow line breaks inside a string, so the key must be a single line with each line
...
3. Paste in the payload below. (Generated from the REST API reference for *Create a Connection* with
   `oracle` as the type, minimized for this lab.)
   - Replace `<your_group_id>` with the `id` from Part 4, Step 6.
   - In `schema_prefix`, replace `firstname_lastname` with your actual first and last name.
   - Replace `ORACLE_PASSWORD` with the password from the 1Password link.
   - Replace `ORACLE_HOSTNAME` with the hostname from the 1Password link.
   - Replace `ORACLE_DATABASE` with the service name or SID from the 1Password link.
   ...
   ```json
   {
     ...
     "config": {
       ...
       "schema_prefix": "firstname_lastname_oracle"
     }
   }
   ```
