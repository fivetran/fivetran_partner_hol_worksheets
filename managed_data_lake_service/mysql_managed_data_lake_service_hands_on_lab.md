# Hands-on Lab: Managed Data Lake Service (MySQL)
...
3. Search for **MySQL** and click **Set up**.
...
5. Select **Fivetran naming**. The name you provide in the **Destination schema prefix** field becomes the name of
   your connector. Use the format `<firstname>_<lastname>_mysql`.
...
6. Populate the setup form with the provided MySQL credentials.
...
`gs://mdls-gcs-hands-on-lab/<firstname>_<lastname>_mdls/<firstname>_<lastname>_mysql_agriculture/<table>/`
...
```
1. You will need to replace `{fivetran_delivered_schema_name}` with name of your connector followed by `agriculture`, so if you've been following the naming conventions it will look something like `firstname_lastname_mysql_agriculture`
...
> `angel_hernandez_mysql` syncing the `agriculture` schema produces the namespace
> `angel_hernandez_mysql_agriculture`. This trips people up — the namespace is not just the
> connector name.
...
```sql
SELECT *
FROM your_database_name.<firstname>_<lastname>_mysql_agriculture.coffee_prices
LIMIT 100;
```
...
CREATE ICEBERG TABLE IF NOT EXISTS <database>.<schema>.coffee_prices
  ...
  CATALOG_NAMESPACE = '<firstname>_<lastname>_mysql_agriculture'
...
| `SHOW ICEBERG TABLES` returns nothing | Namespace in `ALLOWED_NAMESPACES` doesn't match | It is connector name **+ source schema**, e.g. `first_last_mysql_agriculture` — not just the connector name |
...
a MySQL table into it as Apache Iceberg with a Delta transaction log alongside it,
